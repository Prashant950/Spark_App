...
import { useUpdateLifestyleMutation } from "../../services/apiSlice";

const UsersFirstPageLifestyle = () => {
  ...
  const displayName = resolvedAuthUser?.fullName
    ? resolvedAuthUser.fullName.trim()
    : "there";
  ...
  const categories = [
    {
      ...
      icon: <FontAwesome5 name="glass-martini-alt" size={16} color="#A1A1AA" />,
      options: [
        "Not for me",
        "Newly teetotal",
        "Sober curious",
  ...
  // Calculate filled count (e.g., 0/4, 2/4, 4/4)
  const selectedCount = [drink, smoke, exercise, pet].filter(
    (item) => item !== "",
  ).length;
  ...
  const handleSubmitUserFirstPageLifestyle = async () => {
    setLoading(true);
    try {
      const response = await updateLifestyle({ drinking: drink, smoking: smoke, workout: exercise, pets: pet }).unwrap();
      // console.log("Lifestyle update response:", response);
      if (response?.success) {
        await AsyncStorage.setItem(
          "onboardingStep",
          response?.user?.onboardingStep || "LIFESTYLE_COMPLETED"
        );
        router.push("/UserDetails/UsersSecondPageLifeStyle");
      } else {
        console.warn("Lifestyle update did not succeed:", response);
      }
    } catch (error) {
  ...
  const handleSkip = () => {
    router.push("/UserDetails/UsersSecondPageLifeStyle"); // Update with your next screen route
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Scrollable Content with padding at bottom so pets section isn't hidden */}
      <ScrollView
      ...
      >
       ...
       minimumFontScale={0.7}>
          {`Let's talk lifestyle habits, ${displayName}`}
        </Text>
      ...
      {/* Sticky Bottom Next Button with Count (0/4, 2/4, 4/4) */}
      <View style={styles.footer}>
        <TouchableOpacity onPress={handleSubmitUserFirstPageLifestyle} 
...
export default UsersFirstPageLifestyle;
...
const styles = StyleSheet.create({
  ...
  // Scroll Content Body (Extra Bottom Padding to Unhide Pets)
  scrollContent: {
    paddingHorizontal: 20,
    paddingTop: 16,
    paddingBottom: 40,
